...
def evaluate(model, dataloader, tokenizer, mode, DEVICE):
    ...
    model.eval()
    story_loss_fn = nn.NLLLoss(ignore_index=model.tokenizer.pad_token_id, reduction='none')
    story_losses = []
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            for ctx in [emo_ctx, eve_ctx]:
                ...
                outputs['story'] = outputs['story'].permute(0, 2, 1)
                story_loss = story_loss_fn(outputs['story'], dec_in_ids[:, 1:])
